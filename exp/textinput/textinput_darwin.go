...
//   [textInputClient setFrame:NSMakeRect(x, y, 100, 10)];
// }
import "C"
...
func ebitengine_textinput_update(text *C.char, start, end C.int, committed C.BOOL) {
	var c bool
	if committed != 0 {
		c = true
	}
	theTextInput.update(C.GoString(text), int(start), int(end), c)
}
